...
public class AssaultRifle : WeaponBehavior
{
    public int nps = 5;
    ...
    {
        base.Setup(data);
        iWeaponHandle_ = new IAsaulfRifle();
        iWeaponHandle_.Setup(this);

        BYPool pool = new BYPool();
        pool.name_pool = NamePool.BL_HANDGUN;
        pool.total = clipSize;
...
public class IAsaulfRifle : IWeaponHandle
{
    private AssaultRifle wp;
    public void FireHandle()
    {
        wp.audioSource_.PlayOneShot(wp.sfx_fire.OrderBy(x => Guid.NewGuid()).FirstOrDefault());
        Transform bl = PoolManager.instance.Spawn(NamePool.BL_HANDGUN);
